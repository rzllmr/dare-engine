...
class Properties {
    ...
    public register(name: PropertyName, value: string|number|boolean, comment: string, immutable: boolean = false): void {
        if (this.registered.has(name)) throw new Error(`property already registered: ${name}`);

        let property;
        if (typeof(value) === "string")
            property = new Property<string>(name, value, comment, immutable);
        else if (typeof(value) === "number")
            property = new Property<number>(name, value, comment, immutable);
        else 
            property = new Property<boolean>(name, value, comment, immutable);
